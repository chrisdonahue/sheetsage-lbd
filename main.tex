...
We present \emph{Sheet Sage}, 
a system designed to transcribe Western multitrack music into \emph{lead sheets}: human-readable scores which indicate melody and harmony.\footnote{For music with vocals, lead sheets usually also include lyrics. Here we focus just on transcribing the melody and harmony.} 
The potential use cases for reliable lead sheet transcription are broad and include music performance, education, interaction, recommendation, and generation.
However, the problem is challenging because it involves many subtasks: beat tracking, key detection, chord recognition, and melody extraction. 
A major obstacle is melody extraction which, 
while arguably simpler for humans than chord recognition, 
remains a challenging task within MIR. 
...
By combining this new melody extraction system with existing solutions to other subtasks, 
...
decreasing the effort required for performing existing music, 
which in turn may also increase engagement in music education. 
Moreover, such a system could act as a preprocessing component for MIR pipelines which facilitate interaction, recommendation, source separation, and generation. 
Despite the numerous potential benefits, 
such a system does not currently exist as it requires ``everything but the kitchen sink'' for MIR: beat tracking, key detection, chord recognition, and melody extraction.

both perform and understand their favorite songs, or teach 

(1) more easily perform or understand their favorite songs, or (2)
Moreover, such a system could act as a 
While reliably transcribing multitrack music recordings into l
...
% John Thickstun (for conversations)
